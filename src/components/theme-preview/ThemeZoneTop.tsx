/**
 * ThemeZoneTop Component
 * Renders the top zone of the theme (back button, title, badges, settings, etc.)
 */

'use client';

import React from 'react';
import type { TopZone, ThemeItem as IThemeItem, ThemeDef } from '@/lib/theme-parser';
...
import { ButtonItem, TextItem, SpacerItem, TimeLabels, LabelStack } from './ThemeItems';

interface ThemeZoneTopProps {
  zone: TopZone;
  theme: ThemeDef;
}

export function ThemeZoneTop({ zone, theme }: ThemeZoneTopProps) {
  ...
  const slot = zone.slotFor ? zone.slotFor(locked) : (locked ? zone.locked : zone.normal);
  if (!slot || isSlotCompletelyEmpty(slot)) {
    return null;
  }
  ...
  const withPanel = <PanelWrapper content={content} style={panelStyle} />;

  // Wrap in padding and shell
  return (
    <ZoneShell
      content={withPanel}
      vibes={zone.vibes}
...
function buildTitleArea(
  ...
  const titleItems: IThemeItem[] = [];
  const badgeItems: IThemeItem[] = [];
  const stackItems: IThemeItem[] = [];
  const otherItems: IThemeItem[] = [];
 ...
 */
function buildItem(item: IThemeItem, theme: ThemeDef): React.ReactNode | null {
  // Check visibility condition
  const controller = usePlayerControls();
  ...
  switch (item.id) {
    ...
    case 'progress_slider':
      // Progress slider is handled in bottom zone
      return null;
